...
const Order = ({ order }) => {
    ...
    const { id, total, client: { name, surnames, phone, email }, state, client } = order;
    ...
    return (
        <div className={` ${classTipe} border-t-4 mt-4 bg-white rounded p-6 md:grid md:grid-cols-2 md:gap-4 shadow-lg`}>
            <div>
                <p className="font-bold text-gray-800">Cliente: {name} {surnames} </p>
            ...
            <div>
                ...
                {order.order.map(article => (
                    <div key={article.id} className="mt-4">
                        ...
                        <p className="text-sm text-gray-600">Cuantity: {article.cuantity} </p>
                    </div>
                ))}
